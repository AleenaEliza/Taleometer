...
@section('content')
    <div class="page-header">
        <div class="page-leftheader">
            <h4 class="page-title mb-0">Plots</h4>
        </div>
    ...
    <!--End Page header-->
    <!-- Row -->
    @if(checkPermission('/plot','edit') == true)
    <div class="row" id="scrolldiv">
            <div class="col-lg-12 col-md-12">
                <div  class="card">
                    ...
                    @include('admin.masters.plot.details')
                </div>
            </div>
        </div>
        @endif
    ...
    <div class="row">
        <div class="col-12">
            ...
            <div id="" class="main-card mb-3 card">
                <div class="card-header">
                   <div class="col-10">
                     <div class="card-title">Plots List</div>  
                   </div> 
                ...
                <div id="data_content"> 
                @include('admin.masters.plot.list.content')
                </div>
            </div>
            <!--/div-->
    ...
    {{ Form::open(['url' => "plot/delete", 'id' => 'delForm', 'name' => 'delForm'])}}
...
$(document).ready(function(){
    ...
    $("body").on('change','#plotForm #image',function(){  readURL(this); }); 
    ...
    $('body').on('submit','#plotForm',function(e){ 
        $('#plotForm .error').html('');
        if($('#name').val() == ''){ $('#plotForm #name').closest('div').find('.error').html('Enter Plot Name'); return false }
        else if($('#image').val() == '' && !($('#plotForm #id').val() > 0)){ $('#plotForm #image').closest('div').find('.error').html('Select Plot Image'); return false }
        else{ 
            e.preventDefault();    
            var formData = new FormData(this);
            $('#PlotForm #save_btn').attr('disabled',true); $('#PlotForm #save_btn').text('Validating...'); 
            $('#PlotForm #save_btn').text('Saving...'); $('#PlotForm #cancel_btn').trigger('click'); submitForm(formData); return false;
        }
      return false; 
    });

    $('body').on('click','.editPlot',function(){
         $('.card-header h3.card-title').text('Edit Plot'); 
         $('#frontval').val('Edit Plot'); 
        // $('#exampleModal .modal-content').html($('#sizeModal').html());
        $('#plotForm')[0].reset(); $('#plotForm .error').text('');
        $('#image_img').attr('src', "{{url('storage/app/public/default.png')}}");
        var id      =   this.id.replace('editPlot-',''); 
        $.ajax({
            type: "GET",
            url: '{{url("plot")}}/'+id,
            success: function (data) {
                ...
                $.each( data, function( key, value ) { 
                    if(key == 'image')
                    {
                        ...
                        $('#plotForm #'+key).val(value);
                    }
                }); 

                // $('#frontval').text('Update Plot'); 
            } 
        });
        $('html, body').animate({
    ...
    $('body').on('click','.delPlot',function(){ 
        var id      =   this.id.replace('delPlot-',''); $('#delModal #delete_btn').attr('data-id',id);
    ...
    $('body').on('click','.delUser',function(){  
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if(data == "0")
                {
                    toastr.error("Sorry, you can't delete! Audio Stories are mapped to this plot.");
                    setTimeout(function(){ $('#allert_success').fadeOut(); }, 3000);
                    return false;
                }

                var smsg        =   'Plot deleted successfully!';
                toastr.success(smsg);
                $('#data_content').html(data);
                $('#plotForm #id').val(0);
                $('#plotForm')[0].reset();
                $('#image_img').attr('src', "{{url('storage/app/public/default.png')}}");
                $('.card-header h3.card-title').text('Add Plot'); 
                $('#frontval').val('Add Plot'); 
                setTimeout(function(){ $('#allert_success').fadeOut(); }, 3000);
            } 
        });
    ...
    $("body").on("change", ".status-btn", function () {
        ...
        var url         =   '{{url("plot/updateStatus")}}';
        var smsg        =   'Plot activated successfully!';
        if (sts == true){ var status = 1; }else if (sts == false){var status = 0; smsg = 'Plot deactivated successfully!'; }
        updateStatus(id,bId,status,url,'plots','active','is_active',smsg);
    });
...
function submitForm(postValues)
{
    $.ajax({
        ...
        processData: false,
        success: function (data) { 
            $('#plotForm #save_btn').attr('disabled',false); $('#plotForm #save_btn').text('Save');
            msg = 'Plot added successfully!';
            if($('#plotForm #id').val() > 0){
                var msg = 'Plot updated successfully!';
            }
            $('#exampleModal').modal('hide'); 
            $('#plotForm #cancel_btn').trigger('click'); 
            toastr.success(msg);
            $('#data_content').html(data);
            $('#plotForm #id').val(0);
            $('#plotForm')[0].reset();
        ...
        error: function(XMLHttpRequest, textStatus, errorThrown) { 
            ...
            $.each( err.errors, function( key, value ) { 
                // $('#plotForm #'+key).parent().append('<span class="error">'+value+'</span>');
                $('#plotForm #'+key).closest('div').find('.error').html(value);
            }); 
        }
    });
